...
function generatedCatalogSource(entries) {
  ...
  lines.push("  /** Used in dark mode when present, for BOTH render modes — 'original' swaps it in as-is,");
  lines.push("   * 'template' tints it exactly like the light one (e.g. a moon-shaped mask instead of a");
  lines.push("   * sun). Absent means dark mode just re-renders (and, for 'template', re-tints) `light`. */");
...
function scanColorsDir(dir, prefix, entries, warnings) {
  ...
  for (const dirEntry of dirEntries) {
    ...
    // `hasDarkOverride` records whether `dark` was ACTUALLY present in the JSON file — distinct
    // from `dark`'s resolved value below (which defaults to `light` either way, since that's what
    // the generated catalog and useColor() need). Without this, "explicitly set dark equal to
    // light" and "never set dark at all" are indistinguishable once resolved, which broke the
    // editor's "Same as light" / "Custom" toggle: switching to Custom copies light's exact value
    // into dark as a starting point, and without this flag the very next render would see
    // dark === light again and snap the toggle back to "Same as light", hiding the field the user
    // just opened.
    entries.push({ name, light: color.light, dark: color.dark ?? color.light, hasDarkOverride: color.dark !== undefined });
  }
}
...
function generatedAssetsIndexSource(hasColors) {
  ...
  lines.push(" * header comment). Both modes swap in a dedicated `dark` asset when one exists and the");
  lines.push(" * current scheme is dark (a `template` image can use a genuinely different mask shape per");
  lines.push(" * scheme this way — e.g. a sun for light, a moon for dark — not just a different tint on the");
  lines.push(" * same shape), otherwise both fall back to the `light` asset either way. `template` images");
  lines.push(" * (a single-color mask) additionally tint whichever source that resolves to, to `options.color`");
  lines.push(" * if given, else this image's own per-image `tint` override if it has one, else the app-wide");
  lines.push(" * primary tint for the current scheme. `original` images (non-tintable multi-color art) render");
  lines.push(" * untinted.");
  lines.push(" */");
  ...
  lines.push("  const source = useDark ? entry.dark! : entry.light;");
  lines.push("");
  lines.push("  if (mode === 'template') {");
  lines.push("    const perImageTint = entry.tint ? entry.tint[colorScheme] : undefined;");
  lines.push("    return { source, tintColor: options?.color ?? perImageTint ?? _assetCatalogColorPrimaryTint(colorScheme) };");
  lines.push("  }");
  lines.push("");
